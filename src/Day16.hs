...
possibleIndexesForRange range tickets =
  mapMaybe
    ( \idx ->
        let values = S.fromList $ (!! idx) <$> tickets
         in if S.difference values range == S.empty
              then Just idx
              else Nothing
    )
    [0 .. 19]
...
collapseIndices fields
  ...
  | otherwise =
    collapseIndices $
      fmap (\(field, indices) -> (field, filter (`notElem` resolvedIndices) indices)) unresolved
        <> resolved
  where
    ...
    resolvedIndices = concatMap snd resolved

allValidValues :: FieldValues -> S.Set Int
allValidValues =
  M.foldr S.union S.empty
